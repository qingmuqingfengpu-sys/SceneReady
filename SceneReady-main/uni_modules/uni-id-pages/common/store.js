...
export const mutations = {
		...
		if (data) {
			...
			try {
				let res = await usersTable.where("'_id' == $cloudEnv_uid")
					.field('mobile,nickname,username,email,avatar_file,gender,height,wechat_id')
					.get()
	...
	loginBack (e = {}) {
		...
		console.log('loginBack: pages =', pages.map(p => p.route));
		pages.forEach((page, index) => {
			if (pages[pages.length - index - 1].route.split('/')[3] == 'login') {
				delta++
			}
		})
		console.log('loginBack: delta =', delta);
		if (uniIdRedirectUrl) {
		...
		// #ifdef H5
		// H5 平台微信登录也使用 reLaunch 跳转，不使用 history.go
		// if (e.loginType == 'weixin') {
		// 	return window.history.go(-3)
		// }
		// #endif

		// 无论 delta 是否为0，都跳转到首页
		const page = pagesJson.pages[0]
		console.log('loginBack: reLaunch to', `/${page.path}`)
		return uni.reLaunch({
			url: `/${page.path}`
		})
	},
